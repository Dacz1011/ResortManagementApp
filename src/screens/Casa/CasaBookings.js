...
const COLORS = {
  background: '#F7F7F9',
  surface: '#FFFFFF',
  surfaceDark: '#0D3B10',        // Casa Dark Green for Dark Elements
  surfaceDarkActive: '#1B5E20',  // Casa Primary
...
export default function CasaBookings({ route, navigation }) {
    ...
    for (let i = 0; i < days; i++) {
      ...
      if (
        ...
        newBookings[currentDay.getDate()] = {
          ...
          status: 'CONFIRMED',
          amount: `₱${(12000 * days).toLocaleString()}.00`, // Casa Rate
        };
        newlyBookedDates.push(currentDay.getDate());
      }
    }

    addBooking('Casa', newBookings);
  ...
  const handleCancelBooking = () => {
    ...
    Alert.alert(
      'Cancel Booking',
      `Are you sure you want to cancel the booking for ${currentBooking.guestName}?`,
      [
        { text: 'No', style: 'cancel' },
        {
          ...
          style: 'destructive',
          onPress: async () => {
            await deleteStay('Casa', currentBooking.guestName, currentBooking.checkIn, currentBooking.checkOut);
    ...
    return (
      ...
      >
        <View style={styles.heroContainer}>
          <ImageBackground source={{ uri: 'https://images.unsplash.com/photo-1499793983690-e29da59ef1c2?q=80&w=2070&auto=format&fit=crop' }} style={styles.heroImage} imageStyle={styles.heroImageStyle}>
          ...
          <View style={styles.calendarCard}>
            ...
            <View style={styles.daysGrid}>
              {days.map((item, index) => {
                ...
                let cellStyle = [styles.dayCell]; let textStyle = [styles.dayText];
                if (isJustBooked || isSelected) { cellStyle.push(styles.dayCellSelected); textStyle.push(styles.dayTextSelected); }
                else if (isOccupied) { cellStyle.push(styles.dayCellOccupied); textStyle.push(styles.dayTextOccupied); }
                else if (isPast) { textStyle.push(styles.dayTextDisabled); }
              ...
              <View style={styles.detailsCardBody}>
                ...
                <View style={styles.cardFooter}>
                  ...
                  <View style={styles.actionRow}>
                    <TouchableOpacity style={styles.cancelBtn} activeOpacity={0.7} onPress={handleCancelBooking}><X size={20} color={COLORS.textMain} strokeWidth={2.5} /></TouchableOpacity>
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={{ flex: 1 }}>
        <Animated.ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent} bounces={false} style={{ opacity: fadeAnim }}>
          <View style={[styles.heroContainer, { height: 230 }]}>
            <ImageBackground source={{ uri: 'https://images.unsplash.com/photo-1540555700478-4be289fbecef?q=80&w=2070&auto=format&fit=crop' }} style={styles.heroImage} imageStyle={styles.heroImageStyle}>
              <View style={styles.heroOverlay} />
              <View style={[styles.safeArea, { paddingTop: Platform.OS === 'ios' ? insets.top + 10 : StatusBar.currentHeight + 8 }]}>
                <View style={styles.topBar}>
    ...
    <View style={styles.container}>
      ...
      <View style={[styles.bottomNavContainer, { bottom: Platform.OS === 'ios' ? Math.max(insets.bottom + 10, 32) : 24 }]}>
        <View style={styles.bottomNav}>
          ...
          <TouchableOpacity onPress={() => navigation.navigate('CasaFinance')} style={styles.navItem} activeOpacity={0.8}><Wallet size={22} color={activeNav === 'Finance' ? '#FFFFFF' : COLORS.textMuted} /><Text style={[styles.navText, activeNav === 'Finance' && styles.navTextActive]}>Finance</Text></TouchableOpacity>
