...
export default function ReineFinance({ navigation }) {
  ...
  const renderTransactionList = () => (
    <View style={styles.viewContainer}>
      ...
      {/* Floating Action Button (Adjusted for bottom nav) */}
      <TouchableOpacity
        activeOpacity={0.9}
        style={styles.fab}
...
const styles = StyleSheet.create({
  ...
  fab: {
    position: 'absolute',
    bottom: Platform.OS === 'ios' ? 120 : 110, // Avoid bottom nav
    right: 24,
    shadowColor: COLORS.primary,
  ...
  /* Maintenance Log (Solid Deep Pink Card) */
  maintenanceCard: {
  ...
  bottomSpacer: {
    height: 140, // Space for floating nav
  },

  /* --- FLOATING BOTTOM NAV (Matched) --- */
  floatingNavWrapper: {
    position: 'absolute',
