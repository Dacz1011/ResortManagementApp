...
export default function LoginScreen() {
  ...
  useEffect(() => {
    const initLogin = async () => {
      ...
      // Load persisted username if exists
      const savedSession = await mockDb.getAll('auth_session');
      if (savedSession && savedSession.length > 0) {
        setUsername(savedSession[0].lastUser || '');
      }
    };
              ...
              <View style={styles.graphicContainer}>
                ...
                <Animated.View style={[styles.floatingShield, { transform: [{ translateY: floatAnim }] }]}>
                  <View style={styles.shieldInner}>
                    <PropertyIcon size={42} color={activeProp.color} strokeWidth={2} />
                  </View>
                </Animated.View>
              </View>
